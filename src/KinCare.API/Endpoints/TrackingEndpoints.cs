...
public static class TrackingEndpoints
{
        ...
        AppDbContext db)
    {
        ...
        return Results.Content(BuildTrackerHtml(token, ride, residentName, needs), "text/html");
    }
    ...
    private static string BuildTrackerHtml(string token, Ride ride, string residentName, string needs)
    {
        ...
        var actionButton = nextStageInfo is not null
            ? $"<button class=\"btn-action\" style=\"background:{nextStageInfo.ButtonColor}\" onclick=\"advance('{nextStage}')\">"
...
<div class="card" id="action-card">
  __ACTION_BUTTON__
  <button class="btn-report" onclick="reportIssue()">⚠️ Report a problem</button>
